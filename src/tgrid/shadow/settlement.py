...
class SettlementTracker:
    ...
    def __init__(self, policy: object) -> None:
        ...
        self._released: dict = {}  # trade_date -> qty released for sale
    ...
    def record_buy(self, qty: object, *, trade_date: str) -> None:
        ...
        if self._policy.same_day_sellable():
            # T0: the buy is immediately sellable; no lock needed.
            self._released[trade_date] = self._released.get(trade_date, 0) + qty
            return
    ...
    def record_sell(self, qty: object, *, trade_date: str) -> None:
        ...
        (real broker can_use + released shadow) is sufficient.  This method
        debits the released shadow portion only (up to what the shadow owns);
        the remainder, if any, is supplied by real broker can_use and is not
        tracked here.  ``qty`` must be a plain positive int; selling more
        shadow than was released fails closed (AUD-R1-002).
        """
        if type(qty) is not int or qty <= 0:
            raise ShadowInputError("qty must be a positive plain int")
        if type(trade_date) is not str or trade_date == "":
            raise ShadowInputError("trade_date must be a non-empty string")
        available = self.sellable_from_released(trade_date)
        consumed = min(qty, available)
        self._released[trade_date] = available - consumed

    def advance_trading_day(self, from_date: str, to_date: str) -> None:
        """Release the previous day's locked buys (T1) into the new day.

        Under T1, shares bought on ``from_date`` become sellable from
        ``to_date`` (the next eligible trading session).
        """
        ...
        if locked:
            self._released[to_date] = self._released.get(to_date, 0) + locked

    def sellable_from_released(self, trade_date: str) -> int:
        """Sellable quantity derived from released/locked shadow buys."""
        if type(trade_date) is not str:
            raise ShadowInputError("trade_date must be a string")
        return self._released.get(trade_date, 0)
